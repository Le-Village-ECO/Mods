...
namespace Eco.Mods.ERC.PGA.Items
{
    ...
    internal class PortableGeoAnalyzerItem : ToolItem, IInteractor, IHasInteractions, IController, IViewController, IHasUniversalID
    {
        ...
        public void Analyze(Player player, InteractionTriggerInfo triggerInfo, InteractionTarget target)
        {
            if (target.IsBlock && Durability > 0f)
            {
                ...
                locStringBuilder.AppendLineLoc(FormattableStringFactory.Create("In radius {0} found: {1}", new object[]
                {
                    ...
                    Text.Num(GetOilAmount(target.BlockPosition.Value.XZ))
                }));
                LocStringBuilder locStringBuilder2 = locStringBuilder;
                string format = "1x {0} crafting time: {1}";
                object[] array = new object[2];
                int num = 0;
                Item item = Get(typeof(PetroleumItem));
                array[num] = item != null ? new LocString?(item.MarkedUpName) : null;
                array[1] = Text.Num(GetCraftMinutes(player.User, target.BlockPosition.Value));
                    ...
                    foreach (Vector2i vector2i in worldRange.XZIterInc())
                    {
                        ...
                        if (block != null && !(block is EmptyBlock))
                        {
                            ...
                            if (type2 != null && itemsByTag.Contains(type2))
                            {
                                ...
                                {
                                    dictionary2.AddOrUpdate(vector3i.Y, 1, (oldVal, newVal) => oldVal + newVal);
                                }
                                else
                                {
                ...
                int num3 = dictionary.Sum((t) => t.Value.Sum((o) => o.Value));
                if (dictionary.Count > 0)
                {
                    double num4 = Math.Round(num3 / (double)num2 * 100.0, 2);
            ...
            IL_4D2:
                ...
                UseDurability(1f, player);
                BurnCaloriesNow(player, 1f);
                return;
            }
            BurnCaloriesNow(player, 1f);
            if (Durability <= 0f)
            {
                ...
                {
                    Get<BasicBatteryItem>().MarkedUpName
                }));
            }
        }
        ...
        {
            RecipeFamily recipeFamily = CraftingComponent.RecipesOnWorldObject(typeof(PumpJackObject)).FirstOrDefault((rf) => rf.RecipeItems.Any((ri) => ri == typeof(PetroleumItem)));
